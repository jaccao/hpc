...
KnapsackResult knapsackOMP(Node* starter,KnapsackResult b){
    Node* toProcess[65536];
    Node* process = starter->process(b);
    int numberOfNodes=0;
    for(numberOfNodes=0;process;numberOfNodes++){
        if(numberOfNodes >= 65536){
            cout << "Not all nodes was processed!" << endl;
            break;
        }
        toProcess[numberOfNodes]=process;
        if(process){
            if(process == starter){
                process = NULL;
            }else{
...
#pragma omp parallel num_threads(NT)
    {
        ...
        for(int j = idThread;j < numberOfNodes;j += NT){
            Node* processing = toProcess[j];
            KnapsackResult t=knapsackNode(processing,r[idThread]);
            if(r[idThread].maxTotalValue < t.maxTotalValue){
                r[idThread] = t;
            }
            depth[idThread]+=t.depth;
        }
#pragma omp barrier
#pragma omp master
        {
            int rdepth=0;
            for(int t=0;t<NT;t++){
                if(rt.maxTotalValue < r[t].maxTotalValue){
                    rt=r[t];
                }
            }
            for(int t=0;t<NT;t++){
...
KnapsackResult knapsackLevel(NodeRoot* root,KnapsackResult b){
    ...
    KnapsackResult t = knapsackOMP(node, b);
